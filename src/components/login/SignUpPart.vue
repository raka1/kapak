...
async function handleNext() {
  ...
  try {
    const preresponse = await fetch('/api/v1/email/check_email', {
      method: 'POST',
      headers: {
    ...
    const response = await fetch('/api/v1/email/send_verification', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function verifCode(e: Event) {
  ...
  if (target.value.length >= 6) {
    ...
    try {
      const response = await fetch('/api/v1/email/check_code', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
async function finish() {
  ...
  try {
    const response = await fetch('/api/v1/auth/create_user', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
